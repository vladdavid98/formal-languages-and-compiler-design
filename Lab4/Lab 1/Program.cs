...
namespace Lab1
{
    class LanguageSpecification
    {
        ...
        public static List<string> operators = new List<string>() { "+", "-", "*", "/", "%", "<", "<=", "=", ">=", ">",
            ">>", "<<", "==", "&&", "||", "!", "!=", "&", "~",
            "|", "^", "++", "--" };
        public static List<string> reservedWords = new List<string>() { "break", "case", "char", "const", "continue", "default", "do", "double", "else", "float", "for", "cout", "if", "int", "long", "return", "short", "sizeof", "switch",  "void", "while" };
        ...
        public static Dictionary<string, int> Codification
        {
            get
            {
                if (codification == null)
                {
                    ...
                    separators.ForEach(s => { codification[s] = i; i++; });
                    operators.ForEach(s => { codification[s] = i; i++; });
                    reservedWords.ForEach(s => { codification[s] = i; i++; });
    ...
    class Scanner
    {
        ...
        {
            return index == 0 ? false : (line[index - 1] == '\\');
        }
        ...
        public void Scan()
        {
            ...
            foreach (var line in lines)
            {
                ...
                foreach (var token in Scanner.TokenGenerator(line))
                {
                    ...
                    {
                        InternalForm.Add(new InternalFormItem() { Code = LanguageSpecification.Codification[token], Id = -1 });
                    }
                    else if (Scanner.IsIdentifier(token))
                    {
                        if (!Symbols.Values.OfType<string>().Any(i => i == token))
                        {
                            Symbols.Add(id, token);
                            InternalForm.Add(new InternalFormItem() { Code = LanguageSpecification.Codification["identifier"], Id = id.GetHashCode() });
                            id++;
                        }
                    }
                    else if (Scanner.IsConstant(token))
                    {
                        if (!Constants.Values.OfType<string>().Any(i => i == token))
                        {
                            Constants.Add( id, token);
                            InternalForm.Add(new InternalFormItem() { Code = LanguageSpecification.Codification["constant"], Id = id.GetHashCode() });
                            id++;
                        }
                    }
                    else
